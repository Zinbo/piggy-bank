...
var app = {
	...
	calculateItems: function() {
		...
		var weeksDate = new Date(todaysDate.getDate() - todaysDate.getDay());
	...
	calculateIndex: function(start, comparisonDate) {
		...
		for(index = start; index > 0; index--) {
			if(new Date(app.items[index-1].date) >= comparisonDate){
				continue;
			}
			return index;
		}
	...
	populateElements: function() {
		...
		app.populateValue(app.todaysValue, "weekValue");
		app.populateValue(app.todaysValue, "monthValue");
		app.populateValue(app.todaysValue, "yearValue");
	},
